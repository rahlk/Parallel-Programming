  ...
  */
  ...
  while(1)
  {
    ...
    if(rank==1)
    {
      ...
      // Send Row
      MPI_Isend(uc_host + n*(n/2-1) , n/2, MPI_DOUBLE, rank + 1, 2, MPI_COMM_WORLD, &(requestList[reqCount++]));
      // Send Col
      MPI_Isend(col , n, MPI_DOUBLE, rank + 2, 3, MPI_COMM_WORLD, &(requestList[reqCount++]));
      ...
      // uc
      // Receive Row
      MPI_Irecv(uc_host + n/2*(n-2) , n/2, MPI_DOUBLE, rank + 1, 2, MPI_COMM_WORLD, &(requestList[reqCount++]));
      // Receive Col
      MPI_Recv(col , n, MPI_DOUBLE, rank + 2, 3, MPI_COMM_WORLD, &(statusList[statCount++]));
    ...
    if(rank==4)
    {
      ...
      getCol(uc_host, col, n/2, n, 0);
      // Send Row
      MPI_Isend(uc_host + n*(n/2-1) , n/2, MPI_DOUBLE, rank - 1, 2, MPI_COMM_WORLD, &(requestList[reqCount+2]));
      // Send Col
      MPI_Isend(col , n, MPI_DOUBLE, rank - 2, 3, MPI_COMM_WORLD, &(requestList[reqCount+3]));
